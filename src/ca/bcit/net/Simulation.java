...
import ca.bcit.net.algo.CSVUtils;
...
public class Simulation {
	...
	private double regeneratorUsage;
	...
	public void simulate(long seed, int demandsCount, double alpha, int erlang, boolean replicaPreservation, SimulationTask task) throws IllegalAccessException, ClassNotFoundException, InstantiationException {
		...
		try {
			...
			regeneratorUsage = (regeneratorUsage / demandsCount) * 100;
			linkLength /= partCount;
			recordData(erlang);
	...
	private void handleDemand(Demand demand) throws IllegalAccessException, InstantiationException, ClassNotFoundException {
		...
		if (demand.getWorkingPath() != null) {
			regeneratorUsage += demand.getWorkingPath().getOccupiedRegeneratorsPercentage();
			linkLength += demand.getWorkingPath().getParts().stream().mapToInt(s -> s.getLength()).sum();
			partCount += demand.getWorkingPath().getParts().size();
		}
	...
	protected void recordData(int erlang) {
		String csvFile = "D:/Study/Masters/Northeastern/Algorithms/Ceon/ceons-master/US26.csv";
		try {

				FileWriter writer = new FileWriter(csvFile, true);
				CSVUtils.writeLine(writer, Arrays.asList(
						String.valueOf(network.getDemandAllocationAlgorithm().getName()),
						String.valueOf(erlang),
						String.valueOf(this.totalVolume),
						String.valueOf(this.regeneratorUsage),
						//String.valueOf(demand.getWorkingPath().getNeededRegeneratorsCount()),
						String.valueOf((this.regeneratorsBlockedVolume / this.totalVolume) * 100),
						String.valueOf(this.linkLength),
						String.valueOf(this.modulationsUsage[0]),
						String.valueOf(this.modulationsUsage[1]),
						String.valueOf(this.modulationsUsage[2]),
						String.valueOf(this.modulationsUsage[3]),
						String.valueOf(this.modulationsUsage[4]),
						String.valueOf(this.modulationsUsage[5])
				));
				writer.flush();
				writer.close();
